...
use base85::{decode85, encode85};
use rand::Rng;
use spritz::{aead, aead_decrypt, hash, test_aead, test_hash, test_output};
...
fn main() {
  ...
  test_aead(b"ABC", &decode85("Rv0(1hs@aK7O^;R-I4^ss^SC6Q-pB*x!M4&kJm:PyQXV"));
  test_aead(
    b"spam",
...
fn bench_sbox() {
  let mut data = [0; 65535]; // 1048576 is 1MB
  ...
  println!(
    "unbox MB/sec {}",
    (data.len() as f64 / 1048576.0) / toc.elapsed().as_secs_f64(),
  );
}

fn test_sbox(
  expected_boxed: &str,
  expected_unboxed: Result<(String, Vec<u8>),
  &'static str>,
) {
  // """test_scope.keyring
